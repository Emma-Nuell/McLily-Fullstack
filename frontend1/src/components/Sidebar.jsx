...
import { useProductsContext, useUserContext } from "../context/index.js";
...
const Sidebar = () => {
  ...
  const handleClick = (link, item) => {
    if (link.subCategory && !link.route && !item) {
      changeLink(link.name);
    } else if (link.route) {
      navigate(link.route);
      closeSidebar();
    } else if (link.subCategory && link.routes && item) {
      const subRoute = link.routes[item];
                ...
                <ul key={link.id} className="mb-2">
                  ...
                  <ul>
                    {subcategories?.map((item, index) => (
                      ...
                      >
                        <span className="w-full block">{item}</span>
                      </li>
                    ))}
                  </ul>
                </ul>
          ...
          {!isAuthenticated  && (
            <div className="center mt-10 mb-4 flex justify-center">
              <button
                ...
                type="button"
                className="bg-primary-500 px-6 py-4 rounded-md text-black tracking-wide cursor-pointer"
              >
                Login / Sign Up
              </button>
            </div>
