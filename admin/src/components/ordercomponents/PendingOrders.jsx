...
const PendingOrders = () => {
    const { pendingOrders } = useProductContext();
    ...
    const currentProducts = pendingOrders.slice(
      indexOfFirstProduct,
      indexOfLastProduct
    );
    const totalPages = Math.ceil(pendingOrders.length / entriesPerPage);
    ...
    return (
      <div className='bg-white dark:bg-slate-800 p-6 rounded-md mt-12'>
        <div>
          <h2 className='dark:text-red-700 text-red-600 font-bold text-2xl max-sm:text-xl'>
            Pending Orders
          </h2>
        </div>
        ...
        <div className='flex items-center justify-between mt-10 py-4 px-1 gap-6'>
          <div className='text-light-text-secondary dark:text-dark-text-secondary text-sm max-sm:text-xs'>
            ...
            {Math.min(indexOfLastProduct, pendingOrders.length)} of{" "}
